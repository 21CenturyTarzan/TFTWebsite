...
export default function TFTNavbar({
  brand, routes, transparent, light, sticky, relative, center
}) {
  ...
  const renderNavbarItems = routes.map(({ name, icon, href, route, collapse }) => (
    <TFTNavbarDropdown
      ...
      href={href}
      route={route}
      collapse={collapse ? Boolean(collapse) : undefined}
      ...
      onMouseLeave={() => collapse ? setDropdown(null) : undefined}
      light={light}
    />
  ));
  ...
  const renderRoutes = routes.map(({ name, collapse, columns, rowsPerColumn }) => {
    ...
    if (collapse && columns && name === dropdownName) {
      const calculateColumns = collapse.reduce((resultArray, item, index) => {
        const chunkIndex = ~~(index / rowsPerColumn);
  ...
  const dropdownMenu = (
    ...
    >
      {({ TransitionProps }) => (
        ...
        >
          <TFTBox borderRadius="lg">
            <TFTTypography variant="h1" color="white">
              <Icon ref={setArrowRef} sx={{ mt: -3 }}>
                arrow_drop_up
              </Icon>
            </TFTTypography>
            <TFTBox shadow="lg" borderRadius="lg" p={2} mt={2}>
              {renderRoutes}
            </TFTBox>
          </TFTBox>
        </Grow>
  ...
  const nestedDropdownMenu = (
    ...
    >
      {({ TransitionProps }) => (
        ...
        >
          <TFTBox ml={2.5} mt={-2.5} borderRadius="lg">
            <TFTBox shadow="lg" borderRadius="lg" py={1.5} px={1} mt={2}>
              {renderNestedRoutes}
            </TFTBox>
          </TFTBox>
        </Grow>
  ...
  return (
    <Container sx={sticky ? {
      ...
      <TFTBox
        py={0}
        px={{ xs: 4, sm: transparent ? 2 : 3, md: transparent ? 0 : 2 }}
        my={0}
        mx={0}
        width={"100%"}
      ...
      >
        <TFTBox display="flex" justifyContent="space-between" alignItems="center">
          ...
          <TFTBox
            ...
            ml={center ? "auto" : 0}
            mr={center ? "auto" : { xs: "0px", md: "16px" }}
            border={{ xs: "none", md: "2px solid #29455C" }}
            borderRadius="40px"
            sx={{
